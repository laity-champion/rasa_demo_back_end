...
class FindTheCorrespondingDisease(Action):
    ...
    async def run(
        ...
        disease = tracker.get_slot('disease')  # 获取疾病实体' 获取槽值
        print(disease)
        ...
        #  进行判断 输出结果
        if not final_answer:
            dispatcher.utter_message(response='未查询到数据')
        else:
            answer = '\n'.join(final_answer)
            print(answer)
            dispatcher.utter_message(text=answer)
        return [SlotSet('disease', None)]  # 给slot设置值
...
class FindTheCorrespondingsmallTalk(Action):
    ...
    async def run(
        ...
        try:
            ...
            print(e)
            dispatcher.utter_message(response='utter_exception')
        return []
